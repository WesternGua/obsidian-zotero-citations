...
- **脚注 / 尾注 / 文内引用** — 自由切换脚注模式（`^[引用文本]`）、尾注模式（`[^1]` + 文末定义）和文内模式（正文作者-年份引用）
...
执行“文档首选项”打开文档首选项窗口。插件会动态读取 Zotero 中已安装的所有 CSL 样式，以可搜索列表形式展示；选择所需样式后，可同时切换脚注/尾注/文内模式，并一键应用到当前文档的所有引用：
